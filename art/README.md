...
93 件 SVG：卡面插画 67 · 图标 18 · 牌背 3 · 品牌 2 · LOD 示例 3，外加一份字体配置。
全部为本项目原创，许可 CC BY-SA 4.0，见根目录 `LICENSE-ASSETS`。

```
art/
  cards/       67 张卡面：物资 18 · 角色 8 · 天候 10 · 航海 31
  icons/       18 个图标，64×64 母版（其中 5 枚只用于天候牌的效果图示，ADR-0040）
  backs/       3 张牌背：通用 · 秘密 · 物资
  brand/       出处徽记「浪印」（牌面底栏）· 牌背字标（Cinzel 700 的轮廓，不含字体文件）
  lod/         同一张卡的三档 LOD 示例
  fonts.json   字体配置
```
...
游戏不直接用整张卡面。母版烘成三层：**边框**（每种牌每档一张）、**插画**（每张牌一张，透明底），
再由客户端按 `mod/src/main/resources/assets/heavyseas/cards/layout.json` 的槽位图，把牌名、角标、航海牌的口渴排
按当前语言与数据排上去。三档共用同一张槽位图：换档只把东西**拿掉**或**在原地放大**，不挪位置；
哪一档从多宽起，由各槽位最小要多大现算，不写死。

加一张牌只要三样：规则数据、一张透明底插画、语言文件里的牌名 —— 边框、版式、三档都自动继承。
牌面底栏那一格是**出处徽记**：本体印 `brand/emblem.svg`，扩展包印自己的。

## LOD（母版里的分档标记）

同一份 SVG 按显示尺寸分三档，靠 class 控制。**小尺寸下是整条移除元素，不是缩小字号**
—— 9px 的规则文本缩到 29% 是 2.6px，那不是小字，是噪点。下面是母版里的标记，游戏里的分档见上一节。
